...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const motionQuery = respectReducedMotion
      ? (window.matchMedia("(prefers-reduced-motion: reduce)") as MediaQueryListWithDeprecated)
      : null;

    const update = () => {
      if (!mounted) return;
      const meetsWidth = window.innerWidth >= minViewportWidth;
      const allowMotion = motionQuery ? !motionQuery.matches : true;
      setShouldRunSim(meetsWidth && allowMotion);
    };
  ...
  }, [minViewportWidth, respectReducedMotion]);

  const containerStyle = useMemo(() => {
    if (shouldRunSim) return style;
    return {
      background: "radial-gradient(circle at 20% 20%, rgba(82,39,255,0.35), rgba(5,2,24,0.95))",
      ...style,
    };
  }, [shouldRunSim, style]);
